...
func (a *ProcessApiService) GetExecutionExecute(r ProcessApiGetExecutionRequest) (*ExecutionExecution, *http.Response, error) {
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "ProcessApiService.GetExecution")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/process/{process_id}/execution"
	localVarPath = strings.Replace(localVarPath, "{"+"process_id"+"}", url.PathEscape(parameterValueToString(r.processId, "processId")), -1)
...
type ProcessApiGetSnapshotRequest struct {
	ctx context.Context
	ApiService *ProcessApiService
	processId string
}

func (r ProcessApiGetSnapshotRequest) Execute() (*ExecutionSnapshot, *http.Response, error) {
	return r.ApiService.GetSnapshotExecute(r)
}

/*
GetSnapshot Get process snapshot

Get process snapshot

 @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 @param processId The id of the process
 @return ProcessApiGetSnapshotRequest
*/
func (a *ProcessApiService) GetSnapshot(ctx context.Context, processId string) ProcessApiGetSnapshotRequest {
	return ProcessApiGetSnapshotRequest{
		ApiService: a,
		ctx: ctx,
		processId: processId,
...
func (a *ProcessApiService) GetSnapshotExecute(r ProcessApiGetSnapshotRequest) (*ExecutionSnapshot, *http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodGet
		localVarPostBody     interface{}
		formFiles            []formFile
		localVarReturnValue  *ExecutionSnapshot
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "ProcessApiService.GetSnapshot")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/process/:process_id/log/process.sps"
	localVarPath = strings.Replace(localVarPath, "{"+"process_id"+"}", url.PathEscape(parameterValueToString(r.processId, "processId")), -1)
